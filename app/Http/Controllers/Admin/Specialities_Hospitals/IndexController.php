...
namespace App\Http\Controllers\Admin\Specialities_Hospitals;
...
class IndexController extends \App\Http\Controllers\Admin\Controller
{
     ...
     * @return void
     */
    public $get_name = 'speciality_hospital';
    ...
    public function index()
    {
        ...
        if(\Auth::user()->can('admin')) {
            $data['records'] = SpecialityHospital::orderBy('id', 'DESC')->get();
			//$data['doctors'] = SpecialityHospital::permission('user')->get();
        } else {
            $data['records'] = SpecialityHospital::where('created_by', \Auth::user()->id)->orderBy('id', 'DESC')->get();
        }
    ...
    public function addEdit($record_id = null)
    {
        ...
        if( $record_id ) {
			...
			if(\Auth::user()->can('admin')) {
                $data['user'] = SpecialityHospital::where(['id' => $record_id])->first();
            } else {
                $data['user'] = SpecialityHospital::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
            }
            ...
            $data['user'] = new SpecialityHospital;
        }
...
//         return $request->all();
        if(\Auth::user()->can('admin')) {
            $record = SpecialityHospital::where(['id' => $record_id])->first();
        } else {
            $record = SpecialityHospital::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
        }

        if( !$record ) {
            $record = new SpecialityHospital;
        }
        ...
        if( $record instanceof \App\SpecialityHospital ) {
    ...
    {
        if(\Auth::user()->can('admin')) {
            $user = SpecialityHospital::where('id', $id)->first();
        } else {
            $user = SpecialityHospital::where('created_by', \Auth::user()->id)->where('id', $id)->first();
        }
        $user->delete();
    ...
    function importExcel(Request $request){
        ...
        if(isset($data) && $data->count() > 0){
            foreach($data->toArray() as $data){
                if($data['name'] != null){
                    $speciality = SpecialityHospital::where('name', $data['name'])
                                            ->first();
                    ...
                    if(!$speciality){
                        ...
                        if(!$record instanceof SpecialityHospital){
