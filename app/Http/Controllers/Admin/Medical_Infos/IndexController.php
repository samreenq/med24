...
namespace App\Http\Controllers\Admin\Medical_Infos;
...
class IndexController extends \App\Http\Controllers\Admin\Controller
{
    ...
    public function index()
    {
        ...
        if(\Auth::user()->can('admin')){
            $data['records'] = MedicalInfo::with('patient')->orderBy('id', 'DESC')->get();
			//$data['doctors'] = MedicalInfo::permission('user')->get();
        } else {
            $data['records'] = MedicalInfo::with('patient')->where('created_by', \Auth::user()->id)->get();
        }
    ...
    public function detail(Request $request, $userId = null)
    {
        ...
        if(\Auth::user()->can('admin')) {
            $data['record'] = MedicalInfo::permission('user')->where(['id' => $userId])->first();
        } else {
            $data['record'] = MedicalInfo::permission('user')->where(['id' => $userId])->where('created_by', \Auth::user()->id)->first();
        }
...
//            return $data['user']['sessions']->where('gym_id', 3)->sum('total_amount');
    ...
    public function addEdit($record_id = null)
    {
        ...
        if( $record_id ) {
			...
			if(\Auth::user()->can('admin'))
			{
                $data['record'] = MedicalInfo::where(['id' => $record_id])->first();
            }
			else
			{
                $data['record'] = MedicalInfo::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
            }
...
//        $data['user_types'] = ['user' => 'User', 'admin' => 'Admin', 'branch manager' => 'Branch Manager', 'gym owner' => 'Gym Owner'];
    ...
    public function save(Request $request, $record_id = null )
    {
        ...
        if(\Auth::user()->can('admin')) {
            $record = MedicalInfo::where(['id' => $record_id])->first();
        } else {
            $record = MedicalInfo::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
        }
        ...
        if( $record instanceof \App\MedicalInfo )
		{
