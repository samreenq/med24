<?php
namespace App\Http\Controllers\Admin\Family_Members;
...
class IndexController extends \App\Http\Controllers\Admin\Controller
{
    ...
    public function index()
    {
        ...
        if(\Auth::user()->can('admin')){
            $data['records'] = FamilyMember::orderBy('id', 'DESC')->get();
			//$data['doctors'] = FamilyMember::permission('user')->get();
        } else {
            $data['records'] = FamilyMember::where('created_by', \Auth::user()->id)->get();
        }
    ...
    public function detail(Request $request, $userId = null)
    {
        ...
        if(\Auth::user()->can('admin')) {
            $data['record'] = FamilyMember::permission('user')->where(['id' => $userId])->first();
        } else {
            $data['record'] = FamilyMember::permission('user')->where(['id' => $userId])->where('created_by', \Auth::user()->id)->first();
        }
...
//            return $data['user']['sessions']->where('gym_id', 3)->sum('total_amount');
    ...
    public function addEdit($record_id = null)
    {
        ...
        $get_patients = Patient::orderBy('first_name')->get();
		
		foreach($get_patients as $key => $val)
		{
			$data['patients'][$val->id] = 'Patient ID: ' . $val->id . ', ' . $val->first_name . ' ' .  $val->last_name;
		}
        ...
        if( $record_id ) {
			...
			if(\Auth::user()->can('admin'))
			{
                $data['record'] = FamilyMember::where(['id' => $record_id])->first();
            }
			else
			{
                $data['record'] = FamilyMember::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
            }
        ...
        $data['relations'] = Relation::where('status', 1)
...
//        $data['user_types'] = ['user' => 'User', 'admin' => 'Admin', 'branch manager' => 'Branch Manager', 'gym owner' => 'Gym Owner'];
    ...
    public function save(Request $request, $record_id = null )
    {
        ...
        if(\Auth::user()->can('admin')) {
            $record = FamilyMember::where(['id' => $record_id])->first();
        } else {
            $record = FamilyMember::where('created_by', \Auth::user()->id)->where(['id' => $record_id])->first();
        }

        if( !$record_id ) {
            $record = new FamilyMember;
        }
        ...
        if( $record instanceof \App\FamilyMember )
		{
    ...
    {
        if(\Auth::user()->can('admin')) {
            $user = FamilyMember::where('id', $id)->first();
        } else {
            $user = FamilyMember::where('created_by', \Auth::user()->id)->where('id', $id)->first();
        }
        $user->delete();
