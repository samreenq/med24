...
class InsurancesController extends Controller{
    ...
    public function index(){
        ...
        $data['records'] = Insurance::where('type', 'doctor')
    								  ->orderBy('created_at', 'desc')
        							  ->get();

        return view('admin.doctor.'.$this->get_title($this->get_name).'.index', $data);
    }
    ...
    public function addEdit($id = null){
        ...
        return view('admin.doctor.'.$this->get_title($this->get_name).'.add_edit', $data);
    }
    ...
    public function save(Request $request, $id = null){
        ...
        if($insurance instanceof Insurance){
            return redirect()->route('admin.doctors.insurances.edit', $insurance->id)->with('success', 'Data has been saved successfully!');
        }
            ...
            foreach($data->toArray() as $data){
                if($data['medical_center'] != null){
                    ...
                    if(!$record){
                        ...
                        if(!$record->save()){
                            return redirect()->route('admin.doctors.insurances.index')->with('error', 'Something wen\'t wrong');
                        }
                    }
                }
            }

            return redirect()->route('admin.doctors.insurances.index')->with('success', 'Record has been saved successfully!');
        }else{
            return redirect()->route('admin.doctors.insurances.index')->with('error', 'You can\'t submit the empty file');
        }
    }
}