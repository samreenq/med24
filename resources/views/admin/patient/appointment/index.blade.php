...
@section('content')
                ...
                <table class="kt-datatable" id="html_table" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td align="right">{{ $appointment->hospital->name }}</td>
                            ...
                            <td align="right">
                                <a href="{{ route('admin.patient.appointments.update.status', ['id' => $appointment->id, 'status' => 'cancelled']) }}" title="Cancel Appointment" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                    <i class="la la-remove"></i>
                                </a>
                                <a href="{{ route('admin.patient.appointments.update.status', ['id' => $appointment->id, 'status' => 'rejected']) }}" title="Reject Appointment" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                    <i class="la la-ban"></i>
                                </a>
                                <a href="{{ route('admin.patient.appointments.update.status', ['id' => $appointment->id, 'status' => 'pending']) }}" title="Pending Appointment" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                    <i class="la la-question-circle"></i>
                                </a>
                                <a href="{{ route('admin.patient.appointments.update.status', ['id' => $appointment->id, 'status' => 'completed']) }}" title="Complete Appointment" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                ...
                                <a href="{{ route('admin.patient.appointments.update.status', ['id' => $appointment->id, 'status' => 'scheduled']) }}" title="Schedule Appointment" class="btn btn-sm btn-clean btn-icon btn-icon-md">
    ...
    <div class="modal fade" id="saveAppointmentModal" role="dialog" aria-labelledby="appointmentModal" aria-hidden="true">
        <div class="modal-dialog full_modal-dialog modal-lg">
            <div class="modal-content full_modal-content">
                ...
                <form action="{{ route('admin.patient.appointments.saveAppointment') }}" method="post" id="saveAppointment">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <div class="form-group row">
                                <label class="col-form-label col-lg-3 col-sm-12">Hospital *</label>
                                <div class="col-lg-9 col-md-9 col-sm-12">
                                    <div class="input-group">
                                    ...
                                    <span class="form-text text-muted">Please Select Hospital.</span>
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9 col-md-9 col-sm-12">
                                    ...
                                    <span class="form-text text-muted">Please Select Insurance.</span>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.editAppointment', function (e){
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.edit') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
    ...
    $(document).on('change', 'select[name="doctor"]', function (e){
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.getDoctorHospitals') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
                doctorId : $(this).val(),
            ...
            success:function(response){
                if(response.status == 1){
                    $('select[name="hospital"]').append('<option>Select Hospital</option>');

                    $.each(response.data, function (key, value){
                        $('select[name="hospital"]').append('<option value="'+value['id']+'">'+value['name']['en']+'</option>');
                    });

                    if(appointmentData['hospital_id']){
                        $('select[name="hospital"] option[value='+appointmentData['hospital_id']+']').attr('selected', true).change();
                    }
                }else if(response.status == 0){
                    $('select[name="hospital"]').append('<option>No Hospital Found</option>');
                }
            },
            error: function(response, status, error){
                result = $.parseJSON(response.responseText);
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.getDoctorInsurances') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
                doctorId : $(this).val(),
    ...
    $(document).on('change', 'select[name="hospital"]', function (e){
        e.preventDefault();
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.getAvailableDays') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
                doctorId : $('select[name="doctor"] :selected').val(),
                hospitalId : $('select[name="hospital"] :selected').val(),
            },
    ...
    $(document).on('change', 'select[name="patient"]', function (e){
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.getPatientFamilyMembers') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
                patientId : $(this).val(),
    ...
    $(document).on('change', '.datePicker', function (e){
        ...
        $.ajax({
            url : "{{ route('admin.patient.appointments.getAvailableTimeSlots') }}",
            method : "post",
            data : {
                _token : "{{ csrf_token() }}",
                doctorId : $('select[name="doctor"] :selected').val(),
