        ...
        <div class="kt-portlet kt-portlet--mobile">
                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            ...
                            <a href="{{ route('admin.user.' . $menu_active . '.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
            ...
            <div class="kt-portlet__body kt-portlet__body--fit">
                ...
                <table class="kt-datatable" id="html_table" width="100%">
                    <thead>
                        <tr>
                            <th title="Field #1">Customer ID</th>
                            ...
                            <th title="Field #7">Last Session</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $record->id }}</td>
                            <td><a href="{{ route('admin.user.' . $menu_active . '.detail', $record->id) }}">{{ $record->first_name }}</a></td>
                            <td>{{ $record->last_name }}</td>
                            <td>{{ $record->email }}</td>
                            <td>{{ $record->gender }}</td>
                            <td>{{ $record->phone }}</td>
                            <td align="right">{{ $record->status }}</td>
                            <td align="right">{{ ($last_session = $record->sessions->max('start_datetime')) ? date('d M Y H:i:s', strtotime($last_session)) : '' }}</td>
                            ...
                            <td align="right">
                                ...
                                <a href="{{ route('admin.user.' . $menu_active . '.edit', $record->id) }}" title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                    <i class="la la-edit"></i>
                                </a>
                                @endcan
                                @can('delete {{ $menu_active }}')
                                <a href="{{ route('admin.user.' . $menu_active . '.destroy', $record->id) }}" title="Delete" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                            ...
                            <td>
                                @foreach($record->getAllPermissions()->pluck('name') as $index => $permission)
